(function($) {
	...
	//Update Header Style and Scroll to Top
	function supportCenterStyle() {
		if($('.support-center').length){
			var windowpos = $(window).scrollTop();
			var supportButton = document.querySelector('.support-center');
			if (windowpos >= 1) {
				supportButton.textContent = '?';
				supportButton.classList.add('support-center-letter')
			} else {
				supportButton.textContent = 'Centro de Soporte';
				supportButton.classList.remove('support-center-letter')
			}
		}
	}
	...
	if($('.mobile-menu').length){
		...
		// Comprobar datos del LS - Homer Moncayo
		...
		$('.dark-mode .dark-buttons .on').on('click', function() {
			...
			// Añadirlo al localStorage - Homer Moncayo
